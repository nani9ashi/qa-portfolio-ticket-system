...
本リポジトリは SUT（`app/`）と QA 成果物（`qa/`）を同一リポジトリで管理するモノレポであり、テスト対象は同一リポの `app/` 配下のアプリ実装である（シフトレフト構成。[ルートREADME](../../README.md) 参照）。  
同一リポであってもアプリは変更され続けるため、テスト結果・欠陥は **「どのアプリ版（コミット/タグ）」に対するものか** を必ず記録し、再現性を担保する。
...
### 4.2 実行結果（判定）
実行記録の「実行結果」は以下のいずれかで記録する（10_plan の Pass / Fail / Blocked に対応：合格=Pass、不合格=Fail、実行不可=Blocked）。
- 合格：期待値どおり
- 不合格：期待値と差異がある
...
CI実行プロセスの詳細は [40_test_environment.md §3.4](40_test_environment.md#34-ciパイプライン環境) を参照。