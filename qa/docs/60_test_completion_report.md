...
| --- | --- | --- | --- | --- | --- |
| RQ-014 | 未カバー | **B 確定** | Admin作成は対象外（v1.1で確定）。起票責務をRequesterに集約する設計判断。代理起票が必要になった場合は『代理起票ロール』を別建てする想定。 | TCND-034（採用しない） | — |
...
本プロジェクトでは、CI/CDパイプラインにおける品質ゲート（Build Verification）として、**「セキュリティ／バリデーション／認可」の3カテゴリ＋ハッピーパス** に分散した4シナリオを自動化している（v1.1 で1→4に拡張）。
