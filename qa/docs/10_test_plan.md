...
回帰テストの効率化と組合せ網羅のため、自動テストを **2層** で構成する（v1.2 でテストピラミッド最適化）。

- **API/インテグレーション層（runn）：** 認可（RBAC/IDOR）・状態遷移・入力境界の**組合せ網羅**を高速・決定的に検証（10 runbook / 102 step、API-A/B/C）。
- **E2E 層（Playwright）：** 代表シナリオに集約。
  - Auto-01：ハッピーパス（Requesterによる作成と確認）
  - Auto-02：IDOR回帰スモーク（DEFECT-001由来のセキュリティ観点）
  - Auto-04：非担当Agentの認可UI抑止
  - Auto-05：期限過去日の graceful 拒否（DEFECT-003 回帰）
  - ※旧 Auto-03（本文境界値・DEFECT-002 の起点）は v1.2 で **API-C（`validation/title_body_boundary.yml`）へ移設し削除**。
- **実行タイミング：** GitHub への push 時およびプルリクエスト作成時（CI 環境・E2E と runn を並列実行）。
- **詳細：** 実装内容と効果は [テスト完了レポート §11](60_test_completion_report.md#11-付記テスト自動化の実施結果-automated-test-summary)、層の判断は [テスト層戦略（80）](80_test_layer_strategy.md) を参照。
