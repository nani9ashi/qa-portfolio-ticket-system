...
- **対象範囲：** 4シナリオに分散して構成（v1.1で1→4本に拡張）。
  - Auto-01：ハッピーパス（Requesterによる作成と確認）
  - Auto-02：IDOR回帰（DEFECT-001由来のセキュリティ観点）
  - Auto-03：本文境界値（バリデーション、DEFECT-002の起点）
  - Auto-04：非担当Agentの認可UI抑止
- **実行タイミング：** GitHubへのプッシュ時およびプルリクエスト作成時（CI環境）。
- **詳細：** 自動化の実装内容と効果は [テスト完了レポート §11](60_test_completion_report.md#11-付記テスト自動化の実施結果-automated-test-summary) を参照。
