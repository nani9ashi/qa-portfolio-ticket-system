...
私の2つのポートフォリオの中で、本リポジトリは
**QAエンジニアの技術的基礎（CI/CD構築、テスト自動化、Playwright実装）**
を示すものです。

プロダクト品質を戦略的に考える上では、技術的な実装力とプロセス設計の
両輪が必要だと考えています。本リポジトリは前者、
[AI学習レコメンドポートフォリオ](https://github.com/nani9ashi/qa-portfolio-ai-recommender)
は後者（戦略・関係者連携・AI品質）を示しています。
...
## 本ポートフォリオの範囲（明示的なスコープ外）

JSTQB のテスト計画における「説明責任（accountability）」を意識し、**意図的にスコープ外とした項目** を以下に明示します。やらない理由を残すこと自体も品質活動の一部だと考えています。
...
本ポートフォリオは QAエンジニアの基礎技術習得（CI/CD・E2E自動化・JSTQB準拠のドキュメンテーション）を目的としていました。
その後、より戦略的な品質保証・関係者連携・AIプロダクトの品質設計の観点を深めるために、
[AI学習レコメンドポートフォリオ](https://github.com/nani9ashi/qa-portfolio-ai-recommender)
を構築しました。
...
「現場で本当に使えるプロダクトをどう作るか」 に関心を持って学習を続けている社会人です。JSTQB Foundation Level 保有。塾講師や警備員としての経験と、哲学を専攻したバックグラウンドを持ち、**ユーザーや現場目線で仮説を立て、関係者と対話しながら品質を作り込む**スタイルを模索しています。AI プロダクトをはじめとする新しい領域においても、**観察力と論理性**で価値を生み出していきたいと考えています。
