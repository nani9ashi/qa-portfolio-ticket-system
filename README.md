...
複雑な RBAC（ロール認可）と状態遷移を持つ BtoB チケット管理アプリを対象に、JSTQB 準拠のテストプロセス（計画 → 設計 → 実行 → 完了報告）と、手動・API・E2E の3層に配分した自動テスト（CI 品質ゲート）を一人称で完遂したプロジェクトです。開発者として SUT（テスト対象システム）を用意し、QA として保証する一人二役——**このリポジトリの主役は `qa/` 配下の QA 成果物**で、`app/` はその対象です。

題材には原体験があります。施設警備の現場で、シフト管理 SaaS の導入に立ち会いました。機能は揃っているのに、紙の出勤簿は最後まで置き換えられなかった——現場の暗黙の要件が、仕様のどこにも書かれていなかったからです。「動くこと」と「業務で使えること」の間にある品質を、今度は保証する側として作り込む。その最初の一歩が本作です。

**結果として、手動 46 ケース・API 102 ステップ・E2E 4 シナリオを設計・実行し、実バグ 3 件を起票 → 修正 → 回帰まで完遂。** 組合せが爆発する検証を E2E から API 層へ押し下げるテストピラミッドの再配分も、判断根拠ごと文書化しました。
...
| E2E（Playwright） | **4 シナリオ** — 代表導線＋回帰（CI で毎 push 実行） |
| 発見した実バグ | **3件**（DEFECT-001〜003）を起票 → 修正 → 回帰まで完遂 |
| 戦略上の判断 | 組合せ網羅を E2E から API 層へ再配分（テストピラミッド最適化） |
...
- **CI**: GitHub Actions により、push / PR 時に **E2E 4シナリオ**と **runn API（10 runbook）** を並列自動実行し、証跡を保存。
